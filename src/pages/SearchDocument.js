...
import { Col, Row, Button, Form, Breadcrumb, InputGroup } from '@themesberg/react-bootstrap';
...
class SearchDocument extends Component {
  ...
  search(queryText,e) {
      e.preventDefault();
      const archives = [];
      ArchiveService.getAll().orderBy('documentName').startAt(queryText).endAt(queryText + '~').get().then(snapshot => {
          ...
          <Row className="justify-content-between align-items-center">
            <Col xs={8} md={6} lg={3} xl={4}>
              <InputGroup>
                ...
                <Form.Control type="text" placeholder="Masukkan nama dokumen..." name="keyword" onChange={this.onChange}/>
                <Button variant="primary" onClick={e => this.search(this.state.keyword, e)}>Lakukan pencarian</Button>
              </InputGroup>
            </Col>
